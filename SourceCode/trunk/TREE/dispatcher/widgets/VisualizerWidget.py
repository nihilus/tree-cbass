                    ...
                    if(attr.startswith('child')):
                        ...
                        if a is not None:
                            for child in a.split():
                                cur_x = int(float(A.get_node(int(child)).attr['pos'].split(',')[0]))
                                cur_y = int(float(A.get_node(int(child)).attr['pos'].split(',')[1]))
                                node = TextNode(nc, cd, node_dict[str(x)], child, self.t_graph.node[child]['inode'].label(), cur_x, cur_y, 200, 30)
                                self.graphScene.addItem(node)
                                node_dict[child] = node
            #No node exists, meaning this node is not a child of another
            else:
                cur_x = int(float(A.get_node(x).attr['pos'].split(',')[0]))
                cur_y = int(float(A.get_node(x).attr['pos'].split(',')[1]))
