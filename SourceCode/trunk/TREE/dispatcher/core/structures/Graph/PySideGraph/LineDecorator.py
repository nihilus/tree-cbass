...
class LineArrowOnStart(LineDecorator):
    ...
    def calcDecorator(self, line):   
        ...
        sourceArrowP1 = line.p1() + QtCore.QPointF(math.sin(angle + math.pi/ 3) * self.arrowSize,
                                                          math.cos(angle + math.pi / 3) * self.arrowSize)
        sourceArrowP2 = line.p1() + QtCore.QPointF(math.sin(angle + math.pi - math.pi/ 3) * self.arrowSize,
                                                          math.cos(angle + math.pi - math.pi / 3) * self.arrowSize);

        polygons = []
        polygons.append(QtGui.QPolygonF([line.p1(), sourceArrowP1, sourceArrowP2]))
        return polygons
