...
class CornerCalc(LineCalc):
    ...
    def calcEndPoints(self, localSourceRect, localDestRect):    
        ...
        line = QtCore.QLineF(localSourceRect.center() ,localDestRect.center())
        
        #snap to near corners
        sourceOffset = QtCore.QPointF()
        destOffset = QtCore.QPointF()
        ...
        if line.p2().x() > line.p1().x() and line.p2().y() > line.p1().y():
            destOffset = QtCore.QPointF(-destW/2, -destH/2)
            sourceOffset = QtCore.QPointF(sourceW/2, sourceH/2)
        ...
        destPoint = QtCore.QPointF(localDestRect.center().x(), localDestRect.center().y()) + destOffset
        sourcePoint = QtCore.QPointF(localSourceRect.center().x(), localSourceRect.center().y()) + sourceOffset       
...
class CenterCalc(LineCalc):
    ...
    def yClamp(self, outcodeOut, y, ymin, ymax):
        ...
        if outcodeOut & self.TOP :
            val = ymin
        if outcodeOut & self.BOTTOM :
            val = ymax        
        return val
    
    def xClamp(self, outcodeOut, x, xmin, xmax): 
        val = x
        if outcodeOut & self.RIGHT:
            val = xmax
        if outcodeOut & self.LEFT: 
            val = xmin    
        return val    
    ...
    def caclSegmentRectIntersection(self, rect, p1, p2, printd):
        ...
        for i in range(2):
            if y1 - y == 0:
                ...
                outcodeOut = self.computeOutCode(QtCore.QPointF(x,y), rect);
                continue    
            ...
            if outcodeOut & self.TOP : # point is above the clip rectangle
                x = x + slope * (ymin - y)
                y = ymin            
            else:
                if outcodeOut & self.BOTTOM : # point is below the clip rectangle
                    x = x + slope * (ymax - y) 
                    y = ymax
                else:    
                    if outcodeOut & self.RIGHT : # point is to the right of clip rectangle
                        #print "modify", (xmax - x) / slope
                        y = y  +  (xmax - x) / slope
                        x = xmax
                    else:    
                        if outcodeOut & self.LEFT: #    point is to the left of clip rectangle
    ...
    def calcEndPoints(self, srcRect, destRect):
        from PySide import QtGui, QtCore
        #find out line corner origin
        line = QtCore.QLineF(srcRect.center() ,destRect.center())
        
        #both points inside
        code = self.computeOutCode(line.p2(), srcRect)
        if code == self.INSIDE:
            return (QtCore.QPointF(0,0),QtCore.QPointF(0,0))
